...
export function installCameraStreamDiagnostics(accessory: Accessory, console: Console, hevcEnabled: boolean) {
    ...
    writeLog(`HomeKit stream diagnostics build: hevc-fixes-2026-09-16-r40; HEVC option=${hevcEnabled}; legacy RTP=${counts.get('legacy RTP') || 0}; multi-tier RTP=${counts.get('multi-tier RTP') || 0}; WebRTC=${counts.get('WebRTC') || 0}`);
}
