...
export function createCameraStreamSender(console: Console, config: Config, sender: dgram.Socket, ssrc: number, payloadType: number, port: number, targetAddress: string, rtcpInterval: number,
    ...
    let audioTimestampStep = 0;
    if (audioOptions) {
        // Legacy HAP uses its selected capture clock; the new tiers explicitly use 48 kHz.
        const clockRate = audioOptions.rtpClockRate ?? Number(audioOptions.audioSampleRate) * 1000;
        audioTimestampStep = clockRate * audioOptions.audioPacketTime / 1000;
        opusPacketizer = new OpusRepacketizer(audioOptions.framesPerPacket);
    }
    else {
        if (videoOptions.maxPacketSize && videoOptions.codec !== 'h265') {
    ...
    function sendRtcp() {
        if (!packetCount) return; // No valid RTP/NTP mapping exists before the first packet.
        const now = Date.now();
    ...
    function sendPacket(rtp: RtpPacket) {
        ...
        if (now > lastRtcp + rtcpInterval * 1000) sendRtcpInternal(now);
    }
    ...
    function sendRtp(rtp: RtpPacket) {
        ...
        if (audioOptions) {
            ...
            for (const rtp of packets) {
                rtp.header.timestamp = (firstTimestamp + packetCount * audioTimestampStep) >>> 0;
                sendPacket(rtp);
            }
            return;
        }
