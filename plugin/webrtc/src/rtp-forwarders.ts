...
export async function startRtpForwarderProcess(console: Console, ffmpegInput: FFmpegInput, rtpTracks: RtpTracks, options?: {
    ...
    // Cancellation before SDP is expected for an audio source with no packets.
    // This internal promise is not exposed to callers; consume its rejection.
    sdpDeferred.promise.catch(() => {});
    ...
    if (ffmpegInput.url
            ...
            if (audio) {
                if (audioSection) {
                    ...
                    if (!audioSection) {
                        ...
                        const clientIsTcp = await setupRtspClient(console, rtspClient, channel, audioSection, rtspClientForceTcp, rtp => {
        ...
        console.log('video codec/container not matched, transcoding:', videoCodec, {
            ...
            video: ffmpegInput.mediaStreamOptions?.video?.codec,
            audio: ffmpegInput.mediaStreamOptions?.audio?.codec,
        });
    }
    ...
    if (Object.keys(rtpTracks).length) {
        ...
        if (useRtp) {
            ...
            killDeferred.promise.finally(() => serverPort.cancel());

            const packetSizes = Object.values(rtpTracks).map(t => t.packetSize).filter(s => s > 0);
            if (packetSizes.length) args.push('-pkt_size', String(Math.min(...packetSizes)));
            ...
            serverPort.clientPromise.then(async (client) => {
                if (killDeferred.finished) { client.destroy(); return; }
