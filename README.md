...
**r42 is the current known-working release. r43, r44 and r45 are pre-releases under test.**

| Scenario | r42 (known working) | r43 (pre-release) | r44 (pre-release) | r45 (pre-release) |
| --- | --- | --- | --- | --- |
| Live view at home (Multi-Tier RTP, native HEVC up to 4K) | Works | Unchanged | Unchanged | Unchanged |
| Live view away from home at the default 360p (WebRTC through Apple's relay) | Works, with video and audio | Unchanged | Unchanged | Unchanged |
| 1080p away from home | Works: re-encoded at 4 Mbps | Unchanged | Unchanged | Unchanged |
| 4K away from home | Works: the camera's own 4K HEVC stream, sent without re-encoding | Unchanged | Unchanged | Unchanged |
| 1440p away from home | Untested | Untested | Untested | Untested |
| Talkback from the Home app | Not played on the camera | Not played on the camera | Unchanged | Unchanged |
| HomeKit Secure Video recording | Not verified | Not verified | Not verified | Not verified |
| iOS 27 CMAF direct upload | Refused | Refused | Implemented, opt-in; Apple answered 404 | Push AV layout, opt-in, unverified against Apple |
...
r45 answers what the first real r44 session showed: Apple's publishing point returned
HTTP 404 to the object layout r44 guessed and to every other shape probed beside it. r45
uploads the layout the Matter Push AV Stream Transport cluster specifies for CMAF ingest,
which Apple's iOS 27 HAP surface mirrors service for service: one CMAF track file per
track, offered under the Clip ID and then the Session ID, with the status, headers and
body of anything Apple refuses in the console. See [R45-TESTING.md](R45-TESTING.md).

## Install

1. From [Releases](https://github.com/Altair571/homekitnew/releases), download `plugin-hevc-webrtc-rNN.zip` and `Install-Scrypted-Plugin-rNN.command` from the same release into one folder. [r42](https://github.com/Altair571/homekitnew/releases/tag/r42) is known working, and r43, r44 and r45 are pre-releases; earlier releases stay available for rollback.
...
3. Confirm the HomeKit plugin console shows the build, for example `hevc-fixes-2026-09-16-r45`.
...
6. On r44 and r45, optionally turn on **Experimental: HKSV CMAF Direct Upload (r45)** to let the plugin upload HomeKit Secure Video clips to Apple itself. It is off by default; see [R45-TESTING.md](R45-TESTING.md).

Each release's `HEVC-TESTING.md` (in this repository: `R40-TESTING.md` … `R45-TESTING.md`) describes what to check after installing.
...
| Path | Contents |
| --- | --- |
| `plugin/` | Plugin source exactly as the latest build (r45) contains it, apart from the embedded build label. `build-r45-from-r43.py --verify-base` asserts the two match. |
| `tests/` | Node test suite that runs against a built bundle, plus the installer tests |
| `build-r39-from-r38.py` … `build-r45-from-r43.py` | Incremental release builders. Each patches a checksum-locked release ZIP; r45 builds from r43 because r44 was never published as a release. |
| `build-r35-from-r34.py` | Shared helpers the builders import |
| `Install Scrypted Plugin.command` | The installer for the latest packaged release (r45) |
| `R39-TESTING.md` … `R45-TESTING.md` | Release notes and test steps |
...
always `06fc8efce65a8ad03db43c9994eb6832543cb5059158c264f8bab4be1ffbf5d2`, r44 is
always `65e910ad8a71cfe27a98c3feb7564c9ac7d6fddd601c1232ae2b2fc6470a9700` and r45 is
always `7fc5e9c92f59a4c0a3a0b7be819ae3689ea7254a83b2993ac79a57a7a53bf6da`.
...
python3 build-r45-from-r43.py --verify-base       # writes dist-r45/
HK_TEST_BUNDLE=dist-r45/main.nodejs.js node --test --test-concurrency=2 tests/*.test.cjs
python3 tests/test_installer.py                   # checks the packaged r45 ZIP
```

`tests/cmaf-upload.test.cjs` covers the direct upload, driving the whole provisioning
sequence into a publishing point that speaks the Push AV layout and requires the client
certificate the plugin was issued; `tests/cmaf-tracks.test.cjs` covers r45's split of the
recording into CMAF track files, decoding each in FFmpeg. `tests/r43-send-path.test.cjs` covers what r43 changed, including a digest of the
...
`diagnostics/r45-tests.json` records this exact bundle, a zero exit code, and the
hash of `diagnostics/r45-tests.log`, and only if that log shows no failed or skipped
test. Record the run, then package:

```bash
mkdir -p diagnostics
HK_TEST_BUNDLE=dist-r45/main.nodejs.js node --test --test-concurrency=2 tests/*.test.cjs > diagnostics/r45-tests.log; code=$?
python3 - "$code" <<'PY'
import hashlib, json, sys
from pathlib import Path
out = {'bundleSha256': hashlib.sha256(Path('dist-r45/main.nodejs.js').read_bytes()).hexdigest(),
       'returnCode': int(sys.argv[1]),
       'logSha256': hashlib.sha256(Path('diagnostics/r45-tests.log').read_bytes()).hexdigest()}
Path('diagnostics/r45-tests.json').write_text(json.dumps(out, indent=2) + '\n')
PY
python3 build-r45-from-r43.py --package
```
...
```bash
cp "Install Scrypted Plugin.command" Install-Scrypted-Plugin-r45.command
sha256sum plugin-hevc-webrtc-r45.zip Install-Scrypted-Plugin-r45.command > SHA256SUMS-r45.txt
```

Upload those two and the ZIP to the release, with `r45-release-notes.md` as its
description.
