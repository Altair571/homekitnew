...
**r41 is the current known-working release. r42 is a pre-release under test.**

| Scenario | r41 (known working) | r42 (pre-release) |
| --- | --- | --- |
| Live view at home (Multi-Tier RTP, native HEVC up to 4K) | Works | Unchanged |
| Live view away from home (WebRTC through Apple's relay, HEVC) | Works: video and audio at 640×360, 15 fps. Video about 1.6 s and audio about 0.9 s after the stream request. | Unchanged at the default 360p |
| Higher resolution away from home | 1080p is an opt-in setting. It played, but at 1.7 Mbps it looked soft. | 1080p at 4 Mbps, or the camera's own stream unchanged. 1440p and 4K are opt-in and untested. |
...
1. From [Releases](https://github.com/Altair571/homekitnew/releases), download `plugin-hevc-webrtc-rNN.zip` and `Install-Scrypted-Plugin-rNN.command` from the same release into one folder. [r41](https://github.com/Altair571/homekitnew/releases/tag/r41) is known working, and [r42](https://github.com/Altair571/homekitnew/releases/tag/r42) is the pre-release.
2. Run the installer with your Scrypted server address:

   ```bash
   python3 Install-Scrypted-Plugin-r41.command --server https://your-scrypted-host:10443
   ```
...
3. Confirm the HomeKit plugin console shows the build, for example `hevc-fixes-2026-09-16-r41`.
